...
# its polytopes from here; nothing (not even a model-driven eval) writes to it,
# so those answers can't regress from cache poisoning. New polytopes a run
# discovers land in the writable overlay (CYTOOLS_AGENT_KS_CACHE) instead. The
# overlay self-heals: every process prunes dangling fetch records on load.
# Freeze/refresh the base by copying a verified-clean overlay over it.
