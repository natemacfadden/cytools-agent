...
## (2026-07-06) Cut the dead ledger

With the runtime-truth arc dropped, the ledger has no future use, so removed it. It had been write-only and read by nothing since the orchestrator was removed. Deleted cytools_agent/tools/ledger.py; in tools/__init__.py dropped the import, the 13 ledger.wrap(...) calls, and the no-op namespace-rebind loop (kept the `reference` injection into _NS); removed code.py namespace_detail (no callers -- it existed only to dump values for the ledger); reworded the two stale ledger-citing comments in triangulation.py and system_ladder.py. ~160 lines gone, no behavior change: MODEL_TOOLS still 15, run_python/reference work, test_answer 11/11. Trust systems unchanged (offline grading + tools + save_history trace).

## (2026-07-06) Shipped the RAG hybrid into glossary_context

The dense/hybrid retrieval that the bench validated (2026-07-03) now lives in the product. glossary_context switched from keyword-only (_matched_keys) to HYBRID via a new retrieve_keys: keyword matches first (precision floor), then bge-small semantic matches with cosine >= DENSE_THRESHOLD (0.75), capped at max_terms. glossary.py owns the retriever (DENSE_ENABLED/DENSE_THRESHOLD, _ensure_dense, _regex_keys, _dense_keys, retrieve_keys); the model loads lazily on CPU (first call), entries embed once. Dense is OPTIONAL: if sentence-transformers is missing or the load fails, _ensure_dense returns False and it degrades to keyword-only -- the tool never hard-depends on torch. The lint path (expected_by_term/markers) stays keyword-only on purpose, so dense cannot over-fire it.

retrieval_bench.py now imports the SHIPPED functions (regex/dense/hybrid = glossary._regex_keys/_dense_keys/retrieve_keys) instead of its own copy, so the bench measures production and cannot drift. Re-ran it: regex 79%, transformer 48%, hybrid 82% (exact 100, paraphrase 50, corpus 84, false-fire 0) -- identical to the validated numbers, confirming the ship reproduces the benchmark exactly.

Cost accepted (NM): hybrid runs an embedding per user message (bge-small, CPU) plus a one-time model load on the first message. For modest gain (paraphrase 20->50%, recall 79->82%) it is a real per-message cost, mitigated by lazy load + one-time entry embedding + graceful keyword-only fallback; DENSE_ENABLED=False turns it off.
