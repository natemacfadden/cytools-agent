...
ENGINEER_SYSTEM = (
    ...
    "get_polytope_info(ks); print(max(info['genera_2face']))); do not print a "
...
TOOL_CHEATSHEET = (
    ...
    "  get_polytope_info(ks) -> {h11, h21, n_rigid_divisors, genera_2face, "
    "facedim_to_nfaces, ...}\n"
    "  get_heights(ks, n=None, kind='NTFE') -> {shape, heights}; the "
    "triangulations are get_heights(ks)['heights'] (a list of height vectors)\n"
    "  get_cy(ks, heights=None) -> one CY (or a list for many heights)\n"
    "  get_cy_info(ks, heights=None, t=None, cone='Kcup') -> dict (or a list "
    "for many heights); t='tip' adds cy_volume and curve_volumes (a list -- "
    "reduce it yourself, e.g. max(info['curve_volumes']))\n"
    "  get_cy_cones(ks, heights=None, cone='Kcup') -> {mori_rays, "
    "kahler_cone_hyperplanes}\n"
    "For all inequivalent CYs of one polytope: get_cy_info(ks, "
    "get_heights(ks), t='tip') -> a list (aggregate it, e.g. "
